Public Class Form1
    Dim wx As String,
        defaultBrowserFullname As String =
    ...
    Sub 查詢國語辭典()
        ...
        Process.Start(defaultBrowserFullname, url)
#Region "oldCode"
        ...
        'Clipboard.SetText(x)
        'If Len(x) > 1 Then
        '    Shell(Replace(DefaultBrowserFullname, """%1", "http://dict.revised.moe.edu.tw/cgi-bin/newDict/dict.sh?idx=dict.idx&cond=" & 查詢字串轉換_國語會碼(x) & "&pieceLen=50&fld=1&cat=&imgFont=1"))
        'Else
        '    Shell(Replace(DefaultBrowserFullname, """%1", "http://dict.revised.moe.edu.tw/cgi-bin/newDict/dict.sh?idx=dict.idx&cond=^" & 查詢字串轉換_國語會碼(x) & "$&pieceLen=50&fld=1&cat=&imgFont=1"))
        'End If

        'Shell(Replace(DefaultBrowserFullname, """%1", "http://dict.revised.moe.edu.tw/cbdic/search.htm"))
...
#End Region
    ...
    Sub 查詢漢典()
        ...
        Process.Start(defaultBrowserFullname, url)
#Region "舊碼"
        'Shell(Replace(defaultBrowserFullname, """%1", "https://www.google.com.tw/search?q=site:http://www.zdic.net/+" & wx))
        'Shell(Replace(defaultBrowserFullname, """%1", "http://www.zdic.net/search/?q=" & wx)) 'http://www.zdic.net/search/?q=%E8%AD%A6%E7%9B%AE&c=2
#End Region
    End Sub
...
#Region "舊碼"
        ...
        'Shell(Replace(defaultBrowserFullname, """%1", HDurl & wx & "&ka=100&submit="))
        'Shell(Replace(defaultBrowserFullname, """%1", "http://tw.ichacha.net/zaoju/" & wx & ".html")) '查詢查查網造句 2016/10/11
#End Region
    End Sub
    Private Sub Form1_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If qx = "" Then Exit Sub
        wx = 查詢字串轉換_網路碼(qx)
        If Not defaultBrowserFullname.IndexOf("iexplore") Then
            Dim bChrom As New BrowserChrome
            defaultBrowserFullname = bChrom.ChromeAppFileName
        End If
